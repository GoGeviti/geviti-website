import React from 'react'

const Description = () => {
	return (
		<div className='lg:px-3'>
			<div className='container-center'>
				<div className='pb-[89px] pt-16 border-t border-grey-100'>
					<div className='flex items-start justify-between'>
						<div className='w-full'>
							<h3 className='text-4xl font-medium text-primary whitespace-nowrap'>
								Treatment options for <br/><span className='text-grey-primary'>male optimization</span>
							</h3>
						</div>
						<div className='flex flex-col gap-16'>
							<p className='text-grey-primary text-sm'>Geviti takes the guesswork out of health optimization. With AI powered insights, a dedicated care team, and science backed solutions, <br className='max-lg:hidden'/>increasing health span and lifespan has never been easier.</p>
							<div className='flex items-center gap-[88px]'>
								<div>
									<h2 className='text-primary font-medium text-[46px]'>20<span className='text-2xl font-normal'>million</span> </h2>
									<p className='text-grey-primary text-sm'>Men In the united states from ages 25-75 have low T</p>
								</div>
								<div>
									<h2 className='text-primary font-medium text-[46px]'>69%</h2>
									<p className='text-grey-primary text-sm'>Of Adults in the united states are obese or overweight</p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	)
}

export default Description