...
interface NotAloneTransitionProps {
  viewState: ViewState;
  onContinue: () => void;
}

const NotAloneTransition = (props: NotAloneTransitionProps) => {
	useEffect(() => {
	...
	return (
		<Column viewState={ props.viewState }>
			...
			/>
			<Title viewState={ props.viewState }>You&apos;re not alone</Title>
			<Subtitle viewState={ props.viewState }>
				Luckily, this can usually be fixed with the right plan.
			</Subtitle>
		</Column>
	);
};

export default NotAloneTransition;
