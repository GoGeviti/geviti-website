...
const getDataTiers = (isAlreadyOnHRT?: boolean, gender?: string) => {
	...
	if (gender?.toLowerCase() === 'female')
		return onboardingData.pricingPlans.bloodTiersWomen;
...
const PricingPlans: React.FC<PricingPlansProps> = ({
	...
	const [openBiomarkerCompare, setOpenBiomarkerCompare] =
	...
	const getWrapperClassName = (tierIdx: number, mostPopular: boolean) => {
      ...
      hoveredIdx === tierIdx
      	? '!rounded-[30px] ring-2 ring-blue-1 bg-blue-1-background opacity-100'
      	: '!opacity-70';
			...
			<>
					...
					<div className='flex mb-2.5 lg:mb-2'>
						<p
							className={ clsxm(
								...
								hoveredIdx === tierIdx ? 'bg-blue-1' : 'bg-primary/10'
							) }
						>
							{ tier.tag }
						</p>
				...
				<div className='flex items-end justify-between -mb-2.5 relative'>
					<div className='flex flex-col items-start text-left'>
						...
						<span>
							<span className='text-2xl xs2:text-[30px] 2xl:text-[36px] leading-normal -tracking-[0.04em] font-bold'>
								{ tier.price }
							</span>
					...
					{ tier.features.map((feature, featureIdx) => (
						...
						>
							{ /* <div className='flex items-center gap-x-1.5'>
								<QuestionTooltip
									text={ tier.tooltips[featureIdx] }
									className='text-left'
								/>
								<p className='text-base lg:text-xs 2xl:text-base font-medium leading-normal text-[#52585A] -leading-[0.036em]'>{ feature }</p>
							</div> */ }
							<div className='flex items-center gap-1 lg:gap-[6px] text-primary font-medium'>
								<QuestionTooltip
									text={ tier.tooltips[featureIdx] }
									className='text-left'
									tooltipClassName='lg:!left-[-6px]'
								/>
	...
	const renderCompareLabel = () => {
		if (!isAlreadyOnHRT) {
			return (
				...
				>
					<span className='text-primary text-sm font-semibold leading-6'>
						{ onboardingData.pricingPlans.bloodwork.compareLabel }
					</span>
					<svg
						xmlns='http://www.w3.org/2000/svg'
						width='6'
		...
		if (!isAlreadyOnHRT) {
			return (
				<div className='max-xl:hidden flex flex-col items-center text-center text-primary/70 font-BRSonoma text-xs leading-normal font-normal pb-[2.4vh] pt-1.5 relative z-10'>
					{ onboardingData.pricingPlans.bloodwork.disclaimerLabel.map(
						(text: string, textIdx: number) => (
							<span
								key={ textIdx }
					...
					<div className='max-lg:pb-[23px] max-lg:px-6 flex flex-col items-center text-center'>
						<motion.h1
							...
							className={ clsxm(
								...
								isAlreadyOnHRT
									? 'max-w-[430px] 2xl:max-w-[600px] mx-auto'
									: 'max-w-[430px] mx-auto'
							) }
						>
							{ getText().title }
								...
								return (
									<AccordionItem
										...
										className={ clsxm(
											...
											hoveredIdx === tierIdx
												? 'z-10 relative lg:my-0 max-lg:-my-4 shadow-card p-6'
												: 'bg-white lg:my-[4.25vh] max-lg:px-6 max-lg:py-[30px] z-[9]'
										) }
									>
										<AccordionTrigger>
					...
					>
						{ tiers.map((tier: Tier, tierIdx: number) => (
							<motion.div
								...
								initial='initial'
								animate='inView'
								className={ clsxm(hoveredIdx === tierIdx ? 'z-10' : '') }
								onMouseEnter={ () => setHoveredIdx(tierIdx) }
							>
								<div
									className={ clsxm(
										getWrapperClassName(tierIdx, tier.mostPopular),
										'flex flex-col justify-between border-[0.5px] border-grey-background lg:p-[3.7vh] transition-transform duration-750 ease-out',
										hoveredIdx === tierIdx
