						...
						>
							{ howItWorks.list.map((items, id) => (
								<AccordionItem
									key={ id }
									value={ `item-${ id + 1 }` }
									className={ clsxm(id + 1 === howItWorks.list.length ? 'border-none' : 'border-[#EAECF0]', 'py-6') }
								>
									<AccordionTrigger className='flex items-center justify-between w-full space-x-4'>
										<p className='text-[15.614px] md:text-lg leading-[24.289px] md:leading-7 font-Poppins font-medium text-neutral-700 text-start'>{ items.questions }</p>
