			...
			<div className='w-full h-[250px] md:h-[400px] max-md:mt-[60px] relative overflow-hidden'>
				...
				<div className='relative w-full h-full lg:rounded-b-[19px]'>
					<div className='container-center w-full h-full text-center sm:mx-auto flex flex-col items-center justify-center max-md:!max-w-sm'>
						<p
							className={ clsxm(
								'font-BRSonoma text-sm font-semibold tracking-[1.54px] uppercase',
								isPartner ? 'text-primary' : 'text-grey-secondary'
							) }
						>
							{ hero.preTitle }
						</p>
						<div
							className={ clsxm(
								'font-Poppins !text-[30px] md:!text-[40px] mt-[10px] md:mt-5 mb-2 md:mb-4',
								isPartner ? 'text-primary' : 'text-white'
							) }
						>
						...
						{ hero.desc && (
							<div
								className={ clsxm(
									'font-BRSonoma !text-sm',
									isPartner ? 'text-primary' : 'text-grey-secondary'
								) }
							>
								<span dangerouslySetInnerHTML={ { __html: hero.desc } } />
							</div>
						) }
					</div>
				</div>
