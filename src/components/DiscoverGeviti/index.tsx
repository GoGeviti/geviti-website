...
const handleIsElementScrolledIntoHorizontalView = (
  elm: HTMLAnchorElement | null,
...
const DiscoverGeviti: React.FC<DiscoverGevitiProps> = ({
  ...
  React.useEffect(() => {
    products.forEach((_, index) => {
      handleIsElementScrolledIntoHorizontalView(
        itemsRef.current[index],
        wrapperItemsRef.current,
      )
    })
  }, [itemsRef.current])
  ...
  return (
    ...
    >
      <div className="w-full relative">
          ...
          <div className=" md:flex items-center justify-between">
            ...
            <div className="my-6 sm:my-0 flex justify-center items-center bg-white  sm:w-fit gap-[14px] p-1.5 rounded-full">
              <button
                ...
                className={`${
                  activeTab === 'mens-products'
                    ? 'bg-primary text-white shadow-[0px_4px_8px_0px_rgba(0,0,0,0.10)]'
                    : 'text-[#7B7F81]'
                } text-sm md:text-lg cursor-pointer transition-all duration-300  ease-in-out flex  items-center py-3 px-[18px] font-Poppins rounded-full`}
              >
                Mens Products
              </button>
              <button
                onClick={() => setActiveTab('women-products')}
                className={`${
                  activeTab === 'women-products'
                    ? 'bg-primary text-white shadow-[0px_4px_8px_0px_rgba(0,0,0,0.10)]'
                    : 'text-[#7B7F81]'
                } text-sm md:text-lg cursor-pointer transition-all duration-300  ease-in-out flex  items-center py-3 px-[18px] font-Poppins rounded-full`}
              >
        ...
        <div
          className={clsxm(
            'relative wrapper-products-list',
