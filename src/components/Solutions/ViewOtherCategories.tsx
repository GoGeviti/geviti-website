...
const ViewOtherCategories: React.FC<TopTierProps> = ({ slug = 'men-weight-loss' }) => {
	...
	// const [prevSelectedTabIdx, setPrevSelectedTabIdx] = useState<number>(selectedTabIdx);
	...
	useEffect(() => {
		const getContainerOffsetLeft = () => {
			...
			if (containerCenterEl) {
				const computedStyle = getComputedStyle(containerCenterEl);
				const paddingLeft = parseFloat(computedStyle.paddingLeft) || 0;
				setContainerOffsetLeft(containerCenterEl.offsetLeft + paddingLeft);
				setTimeout(() => {
					setIsMounted(true);
				}, 250);
			}
	...
	return (
		...
		>
			...
			<div className='w-full relative mt-[42px] lg:mt-[124px]'>
				<div
					id='container-center'
					className='w-full flex items-center lg:justify-between container-center mb-10'>
					{ renderTabs() }
					{ renderArrowButton() }
				</div>
