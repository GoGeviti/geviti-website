...
const Cards = () => {
	return (
		<>
			...
			<div className='max-lg:-mt-[144px] max-lg:relative lg:hidden w-full flex flex-col items-center'>
				{ processData.map((process, processIdx) => (
					<WrapperCard
						key={ process.id }
						position={ process.position }
...
const WrapperCard = ({ position = 'left', children, id }: any) => {
	return (
		<div
			...
			className={ clsxm(
				'absolute opacity-0',
				position === 'left' && 'lg:top-[240px] lg:left-[134px]',
				position === 'right' && 'lg:top-0 lg:right-[134px]'
			) }
		>
			<div
				className={ clsxm(
