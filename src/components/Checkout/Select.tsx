					...
					<SelectTrigger
						...
						className={
							clsxm(
								...
								size === 'small' && 'h-auto lg:h-auto py-4 pl-4 pr-[10px] text-xs lg:text-xs w-full',
						...
						<span className={ clsxm(
							...
							!value ? 'text-[#AEB1B2]' : 'text-primary',
								...
								<div
									...
									<SelectItem
										...
										className={ clsxm(
											...
											size === 'small' && 'text-xs lg:text-xs rounded-none',
											isLight && 'text-primary bg-white rounded-none data-[highlighted]:bg-[#AEB1B2]',
										) }
									>{ option.label }</SelectItem>
									{ optionIdx < options.length - 1 && (
										<SelectSeparator className={ clsxm(
											isLight ? 'bg-[#AEB1B2]' : 'bg-primary',
										) } />
									) }
								</div>
							)) }
